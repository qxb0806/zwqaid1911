...
class NetworkNovels:
    ...
    def find_novel(self):
        ...
        if soup.select('.odd a'):
            r = self.get_method(soup.select('.odd a')[0]['href'])
            r.encoding='gbk'
            soup=BeautifulSoup(r.text,'html.parser')
        novel_data = soup.select('#list a')
        if not novel_data:
            return None
    ...
    def download_novel(self, datas, choice):
        ...
        with open(download_file, 'w+', encoding='utf-8') as f:
            for chapter in chapters:
                ...
                chapter_url = urljoin(self.URL, chapter['href'])
                try:
                    response = self.get_method(chapter_url)
                    response.encoding = 'gbk'
                    soup = BeautifulSoup(response.text, 'html.parser')
                    content = soup.find('div', id='content').text.replace('\xa0', '').replace('\n','')
                    f.write(content + '\n')
                except:
